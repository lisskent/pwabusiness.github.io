const KEY='earnings_pwa_v16';
const DEFAULT={version:16,user:{name:''},settings:{theme:'light'},activeWorkId:null,works:[],days:{},calendar:{},notes:{},goals:{}};
...
function exportRows(d){
 ...
 tx.forEach(t=>{const c=txCategory(t.categoryId);rows.push({Дата:t.date,Тип:t.type==='expense'?'Расход':'Доход',Категория:c.name,Описание:t.type==='expense'?'Покупка':'Доп. доход',Наличные:t.accountId==='cash'?t.amount:0,Безнал:t.accountId==='card'?t.amount:0,Часы:'',Сумма:t.amount,Счёт:t.accountId==='card'?'Карта':'Наличные',Комментарий:t.note||''})});
