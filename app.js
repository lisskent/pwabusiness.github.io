...
function load(){
  try{
    const keys=[KEY,'earnings_pwa_v18','earnings_pwa_v17','earnings_pwa_v16','earnings_pwa_v14','earnings_pwa_v13','earnings_pwa_v12','earnings_pwa_v11','earnings_pwa_v10','earnings_pwa_v3','earnings_pwa_v1'];
    let raw=null;
    for(const k of keys){
...
function ensureFinance(){
 ...
 data.transactions=data.transactions.filter(t=>t&&t.id).map(t=>({...t,type:t.type==='income'?'income':'expense',amount:Math.max(0,Number(t.amount)||0),date:t.date||today(),categoryId:t.categoryId||(t.type==='income'?'other_income':'other'),accountId:t.accountId||'cash',note:t.note||'',createdAt:t.createdAt||Date.now()}));
 data.transactions.forEach(t=>{if(t.type==='income' && !data.incomeCategories.some(c=>c.id===t.categoryId))t.categoryId='other_income';if(t.type==='expense' && !data.expenseCategories.some(c=>c.id===t.categoryId))t.categoryId='other'});
}
ensureFinance();
