...
function renderTransactionList(){
  const box=document.getElementById('transactionsList');if(!box)return;let tx=monthTransactions(moneyMonthDate).filter(t=>v21TxFilter==='all'||t.type===v21TxFilter);if(v21TxQuery){const q=v21TxQuery.toLowerCase();tx=tx.filter(t=>{const c=txCategory(t.categoryId,t.type);return [c.name,c.icon,t.note,t.date,t.amount].join(' ').toLowerCase().includes(q)})}tx.sort((a,b)=>b.date.localeCompare(a.date)||(b.updatedAt||b.createdAt||0)-(a.updatedAt||a.createdAt||0));const set=(id,v)=>{const e=document.getElementById(id);if(e)e.textContent=v};set('transactionsCount',tx.length);const pageSize=10;const pages=Math.max(1,Math.ceil(tx.length/pageSize));transactionsPage=Math.min(Math.max(1,transactionsPage),pages);const start=(transactionsPage-1)*pageSize;const visible=tx.slice(start,start+pageSize);box.innerHTML=tx.length?visible.map(t=>{const c=t.type==='transfer'?{icon:'↔',name:'Перевод'}:txCategory(t.categoryId,t.type);const amount=t.type==='expense'?'−'+money(t.amount):t.type==='income'?'+'+money(t.amount):money(t.amount);const sub=t.type==='transfer'?`${accountById(t.fromAccountId)?.name||'Счёт'} → ${accountById(t.toAccountId)?.name||'Счёт'}`:`${dateObj(t.date).toLocaleDateString('ru-RU',{day:'numeric',month:'short'})}${t.note?' · '+escapeHtml(t.note):''}`;return `<button class="tx-row" data-tx-edit="${t.id}"><span class="tx-icon">${c.icon||'•'}</span><span class="tx-main"><b>${escapeHtml(c.name)}</b><small>${sub}</small></span><span class="tx-amount ${t.type}">${amount}</span></button>`}).join(''):'<div class="empty-state compact"><b>Ничего не найдено</b><span>Измените поиск или фильтр.</span></div>';box.querySelectorAll('[data-tx-edit]').forEach(b=>b.onclick=()=>openTransaction(b.dataset.txEdit));const pag=document.getElementById('transactionsPagination');if(pag)pag.innerHTML=pages>1?Array.from({length:pages},(_,i)=>{const n=i+1;return `<button class="page-btn ${n===transactionsPage?'active':''}" data-tx-page="${n}">${n}</button>`}).join(''):'';pag?.querySelectorAll('[data-tx-page]').forEach(b=>b.onclick=()=>{transactionsPage=Number(b.dataset.txPage)||1;renderTransactionList();document.getElementById('transactionsList')?.scrollIntoView({behavior:'smooth',block:'start'});});}
...
document.getElementById('txSearch')?.addEventListener('input',e=>{v21TxQuery=e.target.value.trim();transactionsPage=1;renderTransactionList()});document.querySelectorAll('[data-tx-filter]').forEach(b=>b.onclick=()=>{v21TxFilter=b.dataset.txFilter;transactionsPage=1;document.querySelectorAll('[data-tx-filter]').forEach(x=>x.classList.toggle('active',x===b));renderTransactionList()});
